...
%% Stuff special to the book {{{

% one file per chapter
\usepackage{subfiles}

\theoremstyle{definition} %% Don't italicize the theorem statement
...
\numberwithin{Exercise}{chapter} %% Reset to Exercise 1 at each chapter

% make questions use a,b,c instead of 1,2,3 for parts
\renewcommand{\QuestionNB}{\alph{Question}.\ }
\renewcommand{\theQuestion}{\alph{Question}.}

%% make exercises start with a newline
\renewcommand{\ExerciseHeader}{\textbf{\ExerciseName~\ExerciseHeaderNB} (\ExerciseTitle)\smallskip\newline}
\renewcommand{\AtBeginExercise}{\hspace{-0.66em}}

%}}}

% Lie Groups, Algebras, etc

\newcommand*{\GLnC}{\mathsf{GL}(n,\C)}
\newcommand*{\GLnR}{\mathsf{GL}(n,\R)}
\newcommand*{\SLnC}{\mathsf{SL}(n,\C)}
\newcommand*{\SLnR}{\mathsf{SL}(n,\R)}
\newcommand*{\Un}{\mathsf{U}(n)}
...
\todo{mention in the preface that our defns/theorems/cors/lemmas/etc 
all use the same counter to make it easy to find a fixed lemma number}

\todo{mention in the preface that we won't prove everything}

\subfile{chapters/intro}

\subfile{chapters/index}
