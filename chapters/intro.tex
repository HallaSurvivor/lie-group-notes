...
To understand this, it's best to think about the \important{Quaternions}, 
...
\todo{Should we define algebra?}
These come from the $3$ associative \important{Normed Division Algebras}:
...
\missingfigure{The 2d picture of $\mathbb{H}$ and a unit sphere. Page 5 of pdf 4. 
Is there a better place to put this? Should we motivate the image at all? }
...
\begin{proof}
  ...
  The key? Let 
  \todo{we should double check these are correct}

  \todo{Why are these aligned badly?}
  \begin{align*}
...
\missingfigure{A copy of $O(1)$ sitting inside $U(1)$ sitting inside $Sp(1)$.}

In fact, we can define quaternionic analogues of \emph{all} our favorite Lie
groups:

\todo{should we orient this cube differently? It feels a bit weird to me
having the largest thing in the back corner rather than the front corner\ldots
}

\todo{Also, we should redo the other diagrams to be square like this one,
rather than tilted like I did the first time around. I don't have the 
spoons to go back and fix it now, though.}
...
\begin{defn}
  The \important{Quaternionic General Linear Group} is

  \[ \mathsf{GL}(n, \mathbb{H}) = \{ g \in M_n(\mathbb{H}) \mid g \text{ is invertible} \} \]
\end{defn}

\begin{defn}
  The \important{Quaternionic Special Linear Group} is

  \[ \mathsf{SL}(n, \mathbb{H}) = \{ g \in M_n(\mathbb{H}) \mid \text{det}(g) = 1 \} \]
\end{defn}

\begin{defn}
  \todo{I know you had your own name for this, and I think this was it? We should
  say a few more words in the footnote about our choice to call it this.}
  The \important{Quaternionic Unitary Group} is%
...
\todo{Should we say more about the quaternionic determinant? If not, we 
should say that it won't matter since we won't really be needing the
formal definition of these groups in the future. Either way we should
provide some kind of reference.}
