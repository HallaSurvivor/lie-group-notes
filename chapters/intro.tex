...
by considering the closed subgroups of $\mathsf{GL}_n$!
...
\begin{defn}
A \important{Matrix Lie Group} is a closed subgroup of $\mathsf{GL}_n(\C)$. 
\end{defn}
...
We've already met $\mathsf{GL_n}(\R)$, which certainly deserves to be called 
a ``matrix Lie group''. Why is this formally the case, though?

Well this is obviously a subgroup of $\mathsf{GL_n}(\C)$, so we just need to 
show that it's \emph{closed}. So let's fix a sequence of matrices 
$A_n$ converging to $A$ with each $A_n \in \mathsf{GL_n}(\R)$. We want to
show $A$ is in $\mathsf{GL_n}(\R)$ too.
...
real, and so $A \in \mathsf{GL_n}(\R)$.

\subsection{$\mathsf{SL_n}(\C)$}
...
\begin{defn}
  ...
  \[
    \mathsf{SL_n}(\C) = \{ A \in \mathsf{GL_n}(\C) \mid \text{det}(A) = 1 \}
  \]
\end{defn}

\begin{ExerciseList}
  \Exercise[label={ex:SLn-subgroup}] Prove $\mathsf{SL_n}(\C)$ is a subgroup of $\mathsf{GL_n}(\C)$
  (Hint: $\text{det}(AB) = \text{det}(A) \text{det}(B)$)

  \Exercise[label={ex:SLn-closed}] Prove $\mathsf{SL_n}(\C)$ is closed in $\mathsf{GL_n}(\C)$
  (Hint: $\text{det}$ is a continuous function, and $\{1\}$ is closed in $\C$)
\end{ExerciseList}

\subsection{$\mathsf{SL_n}(\R)$}
...
\begin{defn}
  ...
  \[
    \mathsf{SL_n}(\R) = \{ A \in \mathsf{GL_n}(\R) \mid \text{det}(A) = 1 \}
  \]
\end{defn}

Notice $\mathsf{SL_n}(\R) = \mathsf{GL_n}(\R) \cap \mathsf{SL_n}(\C)$ is the 
intersection of closed subgroups of $\mathsf{GL_n}(\C)$, and thus is itself
...
\begin{defn}
  ...
  \[
    \mathsf{U(n)} = \{ A \in \mathsf{GL_n}(\C) \mid A A^* = 1 \}
  \]
...
\begin{ExerciseList}
  \Exercise[label={ex:Un-subgroup}] Prove $\mathsf{U(n)}$ is a subgroup of $\mathsf{GL_n}(\C)$.
...
\begin{lemma}
  For any $g \in \mathsf{GL_n}(\C)$, the following are equivalent:
...
subgroup of $\mathsf{GL_n}(\C)$. In general, if you have a group that acts
...
In this case, $\mathsf{GL_n}(\C)$ acts on $\C^n$ (by matrix-vector multiplication),
...
\begin{defn}
  ...
  \[ 
    \mathsf{O(n)} = \{ A \in \mathsf{GL_n}(\R) \mid AA^* = 1 \}
  \]
...
\begin{defn}
  ...
  \[
    \mathsf{SU(n)} = \{ A \in \mathsf{GL_n}(\C) \mid A A^* = 1 \text{ and } \text{det}(A) = 1 \}
  \]
\end{defn}
...
\begin{defn}
  ...
  \[
    \mathsf{SO(n)} = \{ A \in \mathsf{GL_n}(\R) \mid A A^* = 1 \text{ and } \text{det}(A) = 1 \}
  \]
\end{defn}
